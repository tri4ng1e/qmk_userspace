// Based on getreuer's mouse_turbo_click, with added ability of changing the button in runtime
...
#error "turbo_click: Please set `MOUSEKEY_ENABLE = yes` in rules.mk."
#elif !defined(DEFERRED_EXEC_ENABLE)
#error "turbo_click: Please set `DEFERRED_EXEC_ENABLE = yes` in rules.mk."
#endif

uint16_t tria_turbo_key = KC_BTN1;
bool is_selecting_turbo_keycode = false;
...
#ifndef TURBO_CLICK_PERIOD
#define TURBO_CLICK_PERIOD 20
...
static uint32_t turbo_click_callback(uint32_t trigger_time, void* cb_arg) {
  if (click_registered) {
    unregister_code16(tria_turbo_key);
    click_registered = false;
  } else {
    click_registered = true;
    register_code16(tria_turbo_key);
  }
  return TURBO_CLICK_PERIOD / 2;  // Execute again in half a period.
}
...
bool process_turbo_click(uint16_t keycode, keyrecord_t* record, uint16_t turbo_click_keycode) {
...
bool process_turbo_click_select(uint16_t keycode, keyrecord_t* record, uint16_t select_keycode) {
  if (record->event.pressed) {
    if (is_selecting_turbo_keycode) {
      tria_turbo_key = keycode;
      is_selecting_turbo_keycode = false;
      return false;
    } else if (keycode == select_keycode) {
      is_selecting_turbo_keycode = true;
      return false;
    }
  }
  return true;
